...
public abstract class BaseMeasure : Quantifiable, IBaseMeasure
{
    ...
    public IBaseMeasure? ExchangeTo(Enum measureUnit)
    {
        ...
        if (RateComponentCode == RateComponentCode.Limit && GetDefaultQuantity() % exchangeRate > 0) return null; // ???
    ...
    public IBaseMeasure GetBaseMeasure(string name, ValueType quantity)
    {
        return GetFactory().CreateBaseMeasure(name, quantity);
    }
    ...
    public IBaseMeasure? GetBaseMeasure(Enum measureUnit, decimal exchangeRate, ValueType quantity, string customName)
    {
        return GetFactory().CreateBaseMeasure(measureUnit, exchangeRate, quantity, customName);
    }

    public IBaseMeasure? GetBaseMeasure(string customName, MeasureUnitCode measureUnitCode, decimal exchangeRate, ValueType quantity)
    {
        return GetFactory().CreateBaseMeasure(customName, measureUnitCode, exchangeRate, quantity);
    }
    ...
    {
        return GetDecimalQuantity() * GetExchangeRate();
    }
    #endregion
    #endregion
    ...
    protected static object? GetValidQuantityOrNull(IBaseMeasure baseMeasure, object? quantity)
    {
        ...
        return baseMeasure.RateComponentCode switch
        {
